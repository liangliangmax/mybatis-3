...
public class XMLMapperBuilder extends BaseBuilder {
   ...
   * 所以会出现第一次解析的时候指向的缓存还不存在的情况，所以需要在所有的mapper文件加载完成后进行二次处理，
   * 不仅仅是缓存参考，其他的CRUD也一样。
   ...
   * 解析结果集映射resultMap
   *
   * 结果集映射早期版本可以说是用的最多的辅助节点了，不过有了mapUnderscoreToCamelCase属性之后，如果命名规范控制做的好的话，
   * resultMap也是可以省略的。每个mapper文件可以有多个结果集映射。最终来说，它还是使用频率很高的。
   */
  ...
  * */
   ...
   *
   * 前面讲过，mybatis可以根据不同的数据库执行不同的sql，
   * 这就是通过sql元素上的databaseId属性来区别的。
  ...
  /**
   *
   * 上述过程主要用于获取各个属性，其中唯一值得注意的是processNestedResultMappings，
   * 它用于解析包含的association或collection复合类型,这些复合类型可以使用外部定义的公用resultMap或者内嵌resultMap,
